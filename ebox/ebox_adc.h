...
public:
	Adc(ADC_Type *adcx);
	void begin(void);
	uint16_t analog_read(AdcChnEnum_Type chn);
private:
	ADC_Type * adc;
	
	//ADC通用初始化函数，选择ADCx、采集模式、精度等参数
	uint8_t adc_init(ADC_InitTypeDef);
	//ADC反初始化函数，禁用ADC模块
	uint8_t adc_deinit(ADC_InitTypeDef);
	//使能ADCx相应转换通道的AD采集功能
	uint8_t adc_chn_enable(ADC_Type *, AdcChnEnum_Type);
	//软件触发模式获取AD转换结果
	uint16_t adc_get(ADC_Type *, AdcChnEnum_Type);
	//使能ADCx中断
	uint8_t adc_enable_irq(ADC_InitTypeDef);
	//禁用ADCx中断
	uint8_t adc_disable_irq(ADC_InitTypeDef);
	//使能ADCx转换通道，用于软件中断、硬件触发等模式
	void adc_enable_conversion(ADC_Type *, AdcChnEnum_Type, uint8_t, boolean);
	//获取AD转换结果
	uint16_t adc_get_result(ADC_Type *, uint8_t);
	//获取SC1寄存器中COCO位置1的编号
	uint8_t adc_get_SC1nCOCO(ADC_Type *);
        //DC模块校准函数，内部调用
        uint8_t adc_cal(ADC_Type *adcx);
};
