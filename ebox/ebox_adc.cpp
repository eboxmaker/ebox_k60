...
uint16_t Adc::analog_read(AdcChnEnum_Type chn)
{

	//为了保证复用功能不被占用，最好使能相应ADC通道
	//RUSH开发板的电位器使用DAD1通道进行采集
	//单端DAD1通道的引脚为(ADC0_DP1)
	adc_chn_enable(adc, chn);	

	return adc_get(adc, chn);
 ...
 *    1--配置成功
 */
uint8_t Adc::adc_init(ADC_InitTypeDef adc_init_structure)
{
  uint8_t i;
  ADC_Type *adcx = adc_init_structure.ADC_Adcx;
  uint8_t diff = adc_init_structure.ADC_DiffMode;
  uint8_t mode = adc_init_structure.ADC_BitMode;
  uint8_t time = adc_init_structure.ADC_SampleTimeCfg;
  uint8_t ltime = adc_init_structure.ADC_LongSampleTimeSel;
  uint8_t avg = adc_init_structure.ADC_HwAvgSel;
  uint8_t muxab = adc_init_structure.ADC_MuxSel;
  uint8_t pga = adc_init_structure.ADC_PgaGain;
  uint8_t hwtrg = adc_init_structure.ADC_HwTrgCfg;
 ...
 *    1--配置成功
 */
uint8_t Adc::adc_deinit(ADC_InitTypeDef adc_init_structure)
{
  ADC_Type *adcx = adc_init_structure.ADC_Adcx;
  uint8_t hwtrg = adc_init_structure.ADC_HwTrgCfg;
 ...
 *
 */
uint16_t Adc::adc_get(ADC_Type *adcx, AdcChnEnum_Type chn)
{
 ...
 *
 */
void Adc::adc_enable_conversion(ADC_Type *adcx, AdcChnEnum_Type chn, uint8_t ab, boolean irq)
{
  //参数检查
 ...
 *
 */
uint16_t adc_get_result(ADC_Type *adcx, uint8_t ab)
{
  //参数检查
  ASSERT( ab<=1 );  //判断AB控制寄存器
  return adcx->R[ab];
 ...
 *
 */
uint8_t Adc::adc_get_SC1nCOCO(ADC_Type *adcx)
{
 ...
 *
 */
uint8_t Adc::adc_enable_irq(ADC_InitTypeDef adc_init_structure)
{
  uint8_t i;
  ADC_Type *adcx = adc_init_structure.ADC_Adcx;
 ...
 *
 */
uint8_t Adc::adc_disable_irq(ADC_InitTypeDef adc_init_structure)
{
  uint8_t i;
  ADC_Type *adcx = adc_init_structure.ADC_Adcx;
 ...
 *
 */
uint8_t Adc::adc_chn_enable(ADC_Type *adcx, AdcChnEnum_Type chn)
{
  //判断复用引脚是a或b
  uint8_t mux = (adcx->CFG2 & ADC_CFG2_MUXSEL_MASK)>>ADC_CFG2_MUXSEL_SHIFT;
 ...
 *    1--配置成功
 */
uint8_t Adc::adc_cal(ADC_Type *adcx)
{
  uint16_t cal_var;
