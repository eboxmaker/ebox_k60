 ...
 *    无
 */
void FLASHCLASS::flash_init(void)
{
 ...
 *    错误代码
 */
uint8_t FLASHCLASS::flash_sector_erase(uint32_t FlashPtr)
{
  ...
  FTFE->FCCOB1 = (uint8_t_t) (FlashPtr >> 16);
  FTFE->FCCOB2 = (uint8_t_t)((FlashPtr >> 8 ) & 0xFF);
  FTFE->FCCOB3 = (uint8_t_t)( FlashPtr & 0xFF);
 ...
 *    错误代码
 */
uint8_t FLASHCLASS::flash_byte_program(uint32_t FlashStartAdd, uint32_t *DataSrcPtr, uint32_t NumberOfBytes)
{
  uint8_t Return = FLASH_OK;
  ...
  while ((size_buffer) && (Return == FLASH_OK))
  {
    ...
    FTFE->FCCOB1 = (uint8_t_t)( FlashStartAdd >> 16);
    FTFE->FCCOB2 = (uint8_t_t)((FlashStartAdd >>  8) & 0xFF);
    FTFE->FCCOB3 = (uint8_t_t)( FlashStartAdd & 0xFF);
    //如果K60设置成为小端格式，进行如下操作
    FTFE->FCCOB4 = (uint8_t_t)(*((uint8_t_t*)DataSrcPtr+3));
    FTFE->FCCOB5 = (uint8_t_t)(*((uint8_t_t*)DataSrcPtr+2));
    FTFE->FCCOB6 = (uint8_t_t)(*((uint8_t_t*)DataSrcPtr+1));
    FTFE->FCCOB7 = (uint8_t_t)(*((uint8_t_t*)DataSrcPtr+0));
    FTFE->FCCOB8 = (uint8_t_t)(*((uint8_t_t*)DataSrcPtr+7));
    FTFE->FCCOB9 = (uint8_t_t)(*((uint8_t_t*)DataSrcPtr+6));
    FTFE->FCCOBA = (uint8_t_t)(*((uint8_t_t*)DataSrcPtr+5));
    FTFE->FCCOBB = (uint8_t_t)(*((uint8_t_t*)DataSrcPtr+4));
    //如果K60设置成为大端格式，进行如下操作
    //FTFE_FCCOB4 = (uint8_t_t)(*((uint_8*)DataSrcPtr+0));
    //FTFE_FCCOB5 = (uint8_t_t)(*((uint_8*)DataSrcPtr+1));
    //FTFE_FCCOB6 = (uint8_t_t)(*((uint_8*)DataSrcPtr+2));
    //FTFE_FCCOB7 = (uint8_t_t)(*((uint_8*)DataSrcPtr+3));
    //设置FLASH加载命令
    FTFE->FSTAT |= FTFE_FSTAT_CCIF_MASK;    
    //等待命令完成
    while (!(FTFE->FSTAT & FTFE_FSTAT_CCIF_MASK)) {};
