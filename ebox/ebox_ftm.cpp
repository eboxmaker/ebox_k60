...
#define CPU_MK60DZ10
extern uint32_t g_bus_clock; //to do 
 ...
 *    1--配置成功
 */
uint8_t Ftm::ftm_init(FTM_InitTypeDef ftm_init_structure)
{
  uint8_t result, i;
  //参数检查
  ...
  {
    result = ftm_PWM_init(ftm_init_structure);
  }
  else if(ftm_init_structure.FTM_Mode & FTM_MODE_IC)
  {
    result = ftm_IC_init(ftm_init_structure);
  }
  else if(ftm_init_structure.FTM_Mode & FTM_MODE_QD)
  {
    result = ftm_QD_init(ftm_init_structure);
  }
  else if(ftm_init_structure.FTM_Mode & FTM_MODE_DEC)
  {
    result = ftm_DEC_init(ftm_init_structure);
  }
 ...
 *    1--配置成功
 */
uint8_t Ftm::ftm_deinit(FTM_InitTypeDef ftm_init_structure)
{
  if(ftm_init_structure.FTM_Ftmx == FTM0)
  {
    SIM->SCGC6 &= ~SIM_SCGC6_FTM0_MASK;
  ...
  return ftm_disable_irq(ftm_init_structure);
}
 ...
 *    1--配置成功
 */
uint8_t Ftm::ftm_PWM_enable(FTM_Type *ftmx, FtmChnEnum_Type chn, uint32_t duty, PortPinsEnum_Type pin, uint8_t align)
{
  uint32_t cv;
  uint32_t mod;
 ...
 *    1--配置成功
 */
uint8_t Ftm::ftm_PWM_change_duty(FTM_Type *ftmx, FtmChnEnum_Type chn, uint32_t duty)
{
  uint32_t cv;
  uint32_t mod;
 ...
 *    1--配置成功
 */
uint8_t Ftm::ftm_disable_chn(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
  ftm_pin_deinit(ftmx, chn);
 ...
 *    1--配置成功
 */
uint8_t Ftm::ftm_IC_enable(FTM_Type *ftmx, FtmChnEnum_Type chn, PortPinsEnum_Type pin, uint8_t capture_edge)
{
  if(!ftm_pin_init(ftmx, chn, pin))
    return 0;
 ...
 *
 */
__INLINE boolean Ftm::ftm_is_TOF(FTM_Type *ftmx)
{
 ...
 *
 */
__INLINE void Ftm::ftm_clear_TOF(FTM_Type *ftmx)
{
 ...
 *
 */
__INLINE boolean Ftm::ftm_is_CHnF(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
 ...
 *
 */
__INLINE void Ftm::ftm_clear_CHnF(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
 ...
 *
 */
__INLINE uint16_t Ftm::ftm_get_chn_val(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
 ...
 *
 */
__INLINE uint8_t Ftm::ftm_get_ClkDiv(FTM_Type *ftmx)
{
 ...
 *
 */
__INLINE uint16_t Ftm::ftm_get_counter(FTM_Type *ftmx)
{
 ...
 *
 */
__INLINE void Ftm::ftm_clear_counter(FTM_Type *ftmx)
{
 ...
 *
 */
uint8_t Ftm::ftm_enable_irq(FTM_InitTypeDef ftm_init_structure)
{
  uint8_t i;
 ...
 *
 */
uint8_t Ftm::ftm_disable_irq(FTM_InitTypeDef ftm_init_structure)
{
  uint8_t i;
  FTM_Type *ftmx = ftm_init_structure.FTM_Ftmx;
 ...
 *    1--配置成功
 */
uint8_t Ftm::ftm_QD_enable(FTM_Type *ftmx, PortPinsEnum_Type pha, PortPinsEnum_Type phb)
{
  if(!ftm_pin_init(ftmx, FTM_PhA, pha))
    return 0;
  if(!ftm_pin_init(ftmx, FTM_PhB, phb))
    return 0;
 ...
 *    1--配置成功
 */
uint8_t Ftm::ftm_QD_disable(FTM_Type *ftmx)
{
  if(!ftm_pin_deinit(ftmx, FTM_PhA))
    return 0;
  if(!ftm_pin_deinit(ftmx, FTM_PhB))
    return 0;
 ...
 * FTM模块PWM模式初始化，内部调用
 */
uint8_t Ftm::ftm_PWM_init(FTM_InitTypeDef ftm_init_structure)
{
  uint32_t bus_clk_hz;
  uint32_t mod, mod2;
  uint8_t ps;
  uint32_t freq = ftm_init_structure.FTM_PwmFreq;
  uint32_t dt_en = ftm_init_structure.FTM_PwmDeadtimeCfg;
  uint8_t dt_div = ftm_init_structure.FTM_PwmDeadtimeDiv;
  uint8_t dt_val = ftm_init_structure.FTM_PwmDeadtimeVal;
  FTM_Type *ftmx = ftm_init_structure.FTM_Ftmx;
 ...
 * FTM模块输入捕获模式初始化，内部调用
 */
uint8_t Ftm::ftm_IC_init(FTM_InitTypeDef ftm_init_structure)
{
  uint8_t i;
  uint8_t ps = ftm_init_structure.FTM_ClkDiv;
 ...
 */
uint8_t Ftm::ftm_pin_init(FTM_Type *ftmx, FtmChnEnum_Type chn, PortPinsEnum_Type pin)
{ 
  //根据ftmx使能相应pin的ftm功能
  if(ftmx == FTM0)
  {
 ...
 */
uint8_t Ftm::ftm_pin_deinit(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
 ...
 * FTM模块正交解码模式初始化，内部调用
 */
uint8_t Ftm::ftm_QD_init(FTM_InitTypeDef ftm_init_structure)
{
  uint8_t mode = ftm_init_structure.FTM_QdMode;
  FTM_Type *ftmx = ftm_init_structure.FTM_Ftmx;
...
uint8_t Ftm::ftm_DEC_init(FTM_InitTypeDef ftm_init_structure)
{
