 ...
 *    1--配置成功
 */
uint8_t SPI::spi_init(SPI_InitTypeDef spi_init_structure)
{
  SPI_Type * spix = spi_init_structure.SPI_SPIx;
  uint8_t spi_mode = spi_init_structure.SPI_ModeSelect;
  uint8_t sck_div = spi_init_structure.SPI_SckDivider; 
 ...
 *    1--配置成功
 */
uint8_t SPI::spi_deinit(SPI_InitTypeDef spi_init_structure)
{
  SPI_Type * spix = spi_init_structure.SPI_SPIx;
  spix->MCR |= SPI_MCR_HALT_MASK; //
  if(spix == SPI0)
 ...
 *    1--配置成功
 */
uint8_t SPI::spi_enable_irq(SPI_InitTypeDef spi_init_structure)
{
  SPI_Type * spix = spi_init_structure.SPI_SPIx;
  if(spix == SPI0)
  {
 ...
 *    1--配置成功
 */
uint8_t SPI::spi_disable_irq(SPI_InitTypeDef spi_init_structure)
{
  SPI_Type * spix = spi_init_structure.SPI_SPIx;
  if(spix == SPI0)
  {
 ...
 *    读取从机8位的数据
 */
uint8_t SPI::spi_master_write_read(SPI_Type *spix,uint8_t data,uint8_t pcsx,uint8_t pcs_state)
{
  uint8_t temp;
 ...
 *    读取从机8位的数据
 */
uint8_t SPI::spi_master_read(SPI_Type *spix)
{
  uint8_t temp;

  while(!(spix->SR & SPI_SR_RFDF_MASK)); //=1 RIFO is not empty
  temp=(uint8_t)spix->POPR;
  spix->SR |=SPI_SR_RFDF_MASK;
 ...
 *      |__SPI_PCS_INACTIVE -片选无效,PCS信号变为高电平
 */
void SPI::spi_master_write(SPI_Type *spix,uint8_t data,uint8_t pcsx,uint8_t pcs_state)
{  
