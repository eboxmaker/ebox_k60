...
//RTC使能模块中断
#define rtc_enable_irq()    enable_irq(RTC_IRQn)
//RTC禁止模块中断
#define rtc_disable_irq()   disable_irq(RTC_IRQn)
//RTC获得实时秒计数函数
#define rtc_get_realtime()  (RTC->TSR)
//RTC重新设置秒计数器函数
#define rtc_set_realtime(TIME)     (RTC->TSR=TIME)
//RTC重新设置报警寄存器函数
#define rtc_set_alarm_time(TIME)    (RTC->TAR=TIME)
//RTC关闭函数
#define rtc_stop()         (RTC->SR&=(~RTC_SR_TCE_MASK))


//RTC初始化函数
uint8 rtc_init(RTC_InitTypeDef );
//RTC反初始化函数，关闭选择的RTCx
void rtc_deinit(void );
//判断RTC是否运行函数
uint8 rtc_is_running(void);     
#endif /* __HW_RTC_H__ */
