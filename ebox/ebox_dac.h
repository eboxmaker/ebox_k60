...
{   
  ...
  */
  uint8_t DAC_BufferWatermarkSel;
...
private:
	DAC_Type * dac;
//DAC初始化函数，使能DAC各项功能
uint8_t dac_int(DAC_InitTypeDef);
//DAC反初始化函数，禁用DAC模块
uint8_t dac_deinit(DAC_InitTypeDef);
//使能DACx中断
uint8_t dac_enable_irq(DAC_InitTypeDef);
//禁用DACx中断
uint8_t dac_disable_irq(DAC_InitTypeDef);
//设置DAC缓存区单个数据n
void dac_set_buffer_dataN(DAC_Type *, uint16_t, uint8_t);
//设置DAC缓存区所有数据
void dac_set_buffer_data(DAC_Type *, uint16_t *, uint8_t);
//软件触发DAC缓冲区工作
void dac_software_trigger(DAC_Type *);
};
#endif /* __HW_DAC_H__ */