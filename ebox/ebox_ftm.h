...
uint8_t ftm_init(FTM_InitTypeDef);
//FTM模块通用反初始化
uint8_t ftm_deinit(FTM_InitTypeDef);
//FTM模块PWM模式输出使能，配置输出通道、占空比、指定对应的引脚、对齐方式
uint8_t ftm_PWM_enable(FTM_Type *, FtmChnEnum_Type, uint32_t, PortPinsEnum_Type, uint8_t);
//变更FTM模块PWM模式指定通道的输出占空比
uint8_t ftm_PWM_change_duty(FTM_Type *, FtmChnEnum_Type, uint32_t);
//FTM模块输入捕获模式输出使能，配置输入通道、指定对应的引脚、捕获边缘
uint8_t ftm_IC_enable(FTM_Type *, FtmChnEnum_Type, PortPinsEnum_Type, uint8_t);
//禁用FTM模块指定通道的输出、输入
uint8_t ftm_disable_chn(FTM_Type *, FtmChnEnum_Type);
//FTM模块正交解码模式输入使能，配置输入通道PHA和PHB
uint8_t ftm_QD_enable(FTM_Type *, PortPinsEnum_Type, PortPinsEnum_Type);
//FTM模块正交解码模式输入禁用
uint8_t ftm_QD_disable(FTM_Type *);

//判断FTMx是否产生计数溢出中断标志
boolean ftm_is_TOF(FTM_Type *);
//清除FTMx计数器溢出中断标志
void ftm_clear_TOF(FTM_Type *);
//判断通道n是否产生中断标志
boolean ftm_is_CHnF(FTM_Type *, FtmChnEnum_Type);
//清除通道n中断标志
void ftm_clear_CHnF(FTM_Type *, FtmChnEnum_Type);
//获取FTMx通道n捕获的FTMx计数值
uint16_t ftm_get_chn_val(FTM_Type *, FtmChnEnum_Type);
//获取FTMx时钟分频系数
uint8_t ftm_get_ClkDiv(FTM_Type *);
//获取FTMx计数器值
uint16_t ftm_get_counter(FTM_Type *);
//清空FTMx计数器
void ftm_clear_counter(FTM_Type *);

//使能FTMx中断
uint8_t ftm_enable_irq(FTM_InitTypeDef);
//禁用FTMx中断
uint8_t ftm_disable_irq(FTM_InitTypeDef);

uint8_t ftm_PWM_init(FTM_InitTypeDef);
uint8_t ftm_IC_init(FTM_InitTypeDef);
uint8_t ftm_QD_init(FTM_InitTypeDef);
uint8_t ftm_DEC_init(FTM_InitTypeDef);
uint8_t ftm_pin_init(FTM_Type *, FtmChnEnum_Type, PortPinsEnum_Type);
uint8_t ftm_pin_deinit(FTM_Type *, FtmChnEnum_Type);
